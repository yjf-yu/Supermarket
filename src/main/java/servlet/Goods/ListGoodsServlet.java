...
@WebServlet(name = "ListGoodsServlet",urlPatterns = "/listgoods")
public class ListGoodsServlet extends HttpServlet{
    private static Logger log = LogManager.getLogger (ListGoodsServlet.class.getName ());
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException{
        ...
        try {
            ...
            if (page < 0 || rownumber < 0){
                response.getWriter ().println (Result.PARA_ERROR);
                return ;
            }


            Result result = Goods.getGoodsList (page,rownumber,condition);
